...
use d4::stab::{SimpleKeyValueReader, RangeRecord};
use d4::task::TaskContext;
use regex::Regex;

fn downsample_data(path: &str, chr: &str, mut range: (u32, u32), npoints: usize) -> Result<Vec<(u32, f64)>, Box<dyn std::error::Error>> {
    let mut input: d4::D4FileReader<UncompressedReader, SimpleKeyValueReader<RangeRecord>> = d4::D4FileReader::open(path)?;

    let target = input.header().chrom_list().iter().find(|this|{
        this.name == chr
    }).unwrap();
    ...
    let stat_parts:Vec<_> = (0..npoints).map(|_| {
        let new_end = last_end + base_per_point as u32 + if extra_point == 0 {
            0
        } else {
            extra_point -= 1;
            1
    ...
    Ok(tc.run().into_iter().map(|(_, l, r,b)| ((l + r) / 2,b)).collect())
}
...
pub fn entry_point(args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (chr, left, right) = matches.value_of("region").map(|region| {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption(format!("{}:{}-{}", chr, pos_range.start, pos_range.end), ("sans-serif", (3).percent_height()))
