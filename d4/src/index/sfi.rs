use std::{{io::Write, fs::File}, io::{Error, ErrorKind, Read, Result, Seek}};

use d4_framefile::{Directory, Stream};

use crate::{Header, stab::{RangeRecord, Record, RecordBlockParsingState, SECONDARY_TABLE_METADATA_NAME, SECONDARY_TABLE_NAME, SparseArraryMetadata}};
...
impl SeconaryFrameIndex {
    ...
    pub fn find_partial_seconary_table(
        ...
        if let Some((chr_id, _)) = self.header.chrom_list().iter().enumerate().find(|(_, c)| &c.name == chr) {
            let ret = match self.items.binary_search_by_key(&(chr_id as u32, from), |item| {
    ...
    pub(crate) fn from_reader<R: Read>(mut reader: R, header: Header) -> Result<Self> {
        ...
        let mut ret = SeconaryFrameIndex {
        ...
        for _ in 0..size {
            ...
            let mut item = unsafe {
                *std::mem::transmute::<_, &FrameIndexEntry>(&item_buf[0])
            };
    ...
    pub(crate) fn from_data_track(track_root: &Directory<File>) -> Result<Self> {
        ...
        let stab_metadata = {
            let stream = stab_root.open_stream(SECONDARY_TABLE_METADATA_NAME).unwrap();
        ...
        for stream in stab_metadata.streams() {
            let chrom_id = if let Some((idx, _)) = chrom_list.iter().enumerate().find(|(_, c)| c.name == stream.chr) {
                idx as u32
            } else {
            ...
            let mut state = RecordBlockParsingState::<RangeRecord>::new(stab_metadata.compression());
